...
void OverFindway()
{		
	if(Dir==1&&k3==0)	{pwmcontrol(100);TurnLeft_0();} //小左拐  			 0白  1黑不亮
	else if(Dir==0&&k3==1)	{pwmcontrol(110);TurnRight_1();	}//大右拐
		 else if(Dir==0&&k3==0)	 {pwmcontrol(130);Forward();}	//若检测不到线，直行
			  else if(Dir==1&&k3==1)	{pwmcontrol(130); Forward();}       //	  全黑直行
}
...
void DirAdj()
{	
   	   	   if(LR_1==1&&LR_2==0)	{pwmcontrol(110); TurnLeft_1();}     //  大左拐
	else if(LR_1==0&&LR_2==1)	{pwmcontrol(100); TurnRight_0();}    //	 小右拐
	else if(LR_1==0&&LR_2==0)	{pwmcontrol(130); Forward();}	    //   全亮直行
	else if(LR_1==1&&LR_2==1)	{pwmcontrol(130); Forward();}       //	  全黑直行
...
{	 unsigned char i,j;
	...
	for(i=13;i>0;i--)	//原地左转约90+度
		for(j=60;j>0;j--)
		{pwmcontrol(150);TurnLeft_1();}
	Stop();
	Delay10ms(5);		  

	for(i=40;i>0;i--)	//直走入超车区
		for(j=60;j>0;j--)	   
	...
	for(i=11;i>0;i--)	//原地右转约90度
		for(j=60;j>0;j--)		  
		{pwmcontrol(150);TurnRight_1();}
	Stop();
	Delay10ms(5);

	for(i=43;i>0;i--)	//直走出超车区
		for(j=60;j>0;j--)	   
		{pwmcontrol(150);Forward();}
	Stop();
	Delay10ms(5);

	for(i=10;i>0;i--)	//原地左转约80度
		for(j=60;j>0;j--)		
		{pwmcontrol(150);TurnLeft_1();}			
	Stop();	
	Delay10ms(5);			    

	for(i=30;i>0;i--)      	//超车结束调整姿势
		for(j=60;j>0;j--)	   
		{DirAdj();}	   
	Stop();
	Delay10ms(30);				   //超车完成 
